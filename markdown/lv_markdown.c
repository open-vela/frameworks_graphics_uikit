    ...
    if(ctx->current_obj == NULL || ctx->block_type != MARKDOWN_BLOCK_TYPE_TEXT) {
        ...
        switch(ctx->block_type) {
            case MARKDOWN_BLOCK_TYPE_TEXT: {
                    ...
                    if(mark->paragraph_style_cb) mark->paragraph_style_cb(&mark->paragraph_style);
                    else default_markdown_paragraph_style_cb(&mark->paragraph_style);
            ...
            case MARKDOWN_BLOCK_TYPE_THEMATIC_BREAK: {
                    ...
                    if(mark->thematic_break_style_cb) mark->thematic_break_style_cb(&mark->thematic_break_style);
                    else default_markdown_thematic_break_style_cb(&mark->thematic_break_style);
...
static void exit_block(lv_markdown_t * mark)
{
    ...
    int32_t offset = 0;
    if(!ctx->is_inline) offset = lv_obj_get_height(ctx->current_obj) + 15;
...
void add_text(lv_markdown_t * mark, const char * text)
{
    ...
    if(ctx->link_url || ctx->link_title) {
        ctx->link_alt = text;
        if(mark->url_create_cb) mark->url_create_cb(span, ctx->link_url, ctx->link_title, ctx->link_alt);
        else default_markdown_url_create_cb(span, ctx->link_url, ctx->link_title, ctx->link_alt);
    }
    else {
        ...
        else lv_span_set_text(span, text);
    }

    if(mark->heading_style_cb) mark->heading_style_cb(&span->style, ctx->heading_level);
...
static void render_node(cmark_node * node, cmark_event_type ev_type, lv_obj_t * obj, int32_t options)
{
    ...
    switch(type) {
        ...
        case CMARK_NODE_ITEM: {
                ...
                else default_markdown_list_marker_cb(list_marker, LIST_MARKER_SIZE,
                                                         ctx->list_level, list_index, parent_type == CMARK_ORDERED_LIST);
                if(entering) {
                    ctx->inline_offset = 0;
                    ctx->is_inline = true;
        ...
        case CMARK_NODE_TEXT: {
                ...
                if(ctx->block_type != MARKDOWN_BLOCK_TYPE_TEXT || ctx->unsupported_level > 1) break;
        ...
        case CMARK_NODE_STRONG: {
                if(entering) ctx->strong_level++;
                else ctx->strong_level--;
                break;
            }

        case CMARK_NODE_EMPH: {
                if(entering) ctx->emphasis_level++;
                else ctx->emphasis_level--;
                break;
            }
...
{
    LV_UNUSED(level);
    if(ordered) lv_snprintf(buff, size, "%d. ", index);
    else lv_snprintf(buff, size, level == 1 ? "• " : "- ");
}
static void default_markdown_thematic_break_style_cb(lv_style_t * style)
{
