...
static PS* PS_construct(const char* commands, int commandCount, const float* points, int pointCount)
{
    ...
    while (c < commandCount) {
        ...
        float sg = points[p++];
        float sb = points[p++];
        ps->m_paths[i].m_scolor = lv_color_make((uint8_t)(sr * 255.0f), (uint8_t)(sg * 255.0f), (uint8_t)(sb * 255.0f));

        float fr = points[p++];
        float fg = points[p++];
        float fb = points[p++];
        ps->m_paths[i].m_fcolor = lv_color_make((uint8_t)(fr * 255.0f), (uint8_t)(fg * 255.0f), (uint8_t)(fb * 255.0f));
...
static void draw_animation(lv_obj_t* obj, lv_layer_t* layer)
{
    ...
    for (int i = 0; i < ps->m_numPaths; i++) {
        ...
        lv_vector_dsc_set_stroke_miter_limit(ctx, (int32_t)ps->m_paths[i].m_miterLimit);
        lv_vector_dsc_set_stroke_width(ctx, ps->m_paths[i].m_lineWidth);
