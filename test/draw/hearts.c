...
static void create_particles(lv_hearts_t* heart)
{
    ...
    /*init path*/
#if CLOVER
    vg_vector_shapes_factory(VG_VECTOR_SHAPE_CLOVER, heart->path);
#else
    vg_vector_shapes_factory(VG_VECTOR_SHAPE_HEART, heart->path);
#endif
}
...
void uikit_draw_demo_hearts(char* info[], int size, void* param)
{
