 ...
 *      INCLUDES
 *********************/
#include "object_creation_test.h"
...
static inline void get_used_time_render_label(const int num, lv_obj_t* parent)
{
    ...
    LV_PROFILER_BEGIN;
    for (int i = 0; i < num; i++) {
        const lv_font_t* font = vg_font_create(test_case_fonts[i].name, test_case_fonts[i].size, test_case_fonts[i].style);
...
/****************************************************************************
 * uikit_demo_time_render_spandgroup
 ****************************************************************************/
