 ...
 *      DEFINES
 *********************/
#define MY_CLASS &video_controller_demo_class
 ...
 *  STATIC PROTOTYPES
 **********************/
static void video_controller_demo_constructor(const lv_obj_class_t* class_p, lv_obj_t* obj);
static void video_controller_demo_destructor(const lv_obj_class_t* class_p, lv_obj_t* obj);
...
static void video_controller_set_callback(video_controller_demo_t* video_controller);

/**********************
 *  STATIC VARIABLES
 **********************/
const lv_obj_class_t video_controller_demo_class = {
    .constructor_cb = video_controller_demo_constructor,
    .destructor_cb = video_controller_demo_destructor,
    .instance_size = sizeof(video_controller_demo_t),
...
lv_obj_t* uikit_demo_video_controller_create(lv_obj_t* parent)
{
    ...
    video_controller_demo_t* video_controller = (video_controller_demo_t*)obj;
...
static inline void video_controller_set_callback(video_controller_demo_t* video_controller)
{
    vg_video_set_callback(video_controller->video, MEDIA_EVENT_PREPARED, video_controller, prepared_cb);
    vg_video_set_callback(video_controller->video, MEDIA_EVENT_COMPLETED, video_controller, completed_cb);
    vg_video_set_callback(video_controller->video, MEDIA_EVENT_STOPPED, video_controller, stopped_cb);
}
...
static void video_event_cb(lv_event_t* e)
{
    ...
    video_controller_demo_t* video_controller = (video_controller_demo_t*)lv_event_get_user_data(e);
...
static void play_pause_event_cb(lv_event_t* e)
{
    ...
    video_controller_demo_t* video_controller = lv_event_get_user_data(e);

    if (code == LV_EVENT_SHORT_CLICKED) {
        if (lv_obj_has_state(video_controller->play_imgbtn, LV_STATE_CHECKED)) {
            if (video_controller->is_stop) {
                vg_video_set_src_opt(video_controller->video, video_controller->src, video_controller->option);
                video_controller_set_callback(video_controller);
                video_controller->is_stop = false;
            }
            vg_video_resume(video_controller->video);
        } else {
            vg_video_pause(video_controller->video);
        }
    } else if (code == LV_EVENT_LONG_PRESSED) {
        if (!video_controller->is_stop) {
            vg_video_stop(video_controller->video);
            video_controller->is_stop = true;
        } else {
...
{
    video_controller_demo_t* video_controller = (video_controller_demo_t*)obj;
    vg_video_get_dur(video_controller->video, duration_received_cb, video_controller);
}
...
static void completed_cb(void* obj)
{
    ...
    vg_video_pause(video_controller->video);
    vg_video_seek(video_controller->video, 0);
}
