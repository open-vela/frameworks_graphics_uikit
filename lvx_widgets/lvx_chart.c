/**
 * @file lvx_chart.c
 * @todo part_draw_dsc.text part_draw_dsc.class_p part_draw_dsc.type. These
 * three data are only available in the latest version, and now temporarily
 * annotate
 */
                                   ...
                                   uint16_t id, lv_point_t* p_out)
{
    ...
    if (chart->type == LVX_CHART_TYPE_LINE) {
        ...
        int32_t ser_gap
            = (lv_obj_get_style_pad_column(obj, LV_PART_ITEMS) * chart->zoom_x)
            >> 8; /*Gap between the column on the ~same X*/
        int32_t block_gap
            = (lv_obj_get_style_pad_column(obj, LV_PART_MAIN) * chart->zoom_x)
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        if (_lv_ll_is_empty(&chart->series_ll) == false) {
            if (chart->type == LVX_CHART_TYPE_LINE)
                ...
                draw_series_point_bar(obj, clip_area);
#endif
        }
        ...
        for (i = 0; i < chart->point_cnt; i++) {
            ...
            if (i != 0) {
                if (crowded_mode) {
                        ...
                        if (p1.x != p2.x) {
                            ...
                            if (p1.y == p2.y)
                                p2.y++; /*If they are the same no line will be
                                           drawn*/
                            lv_draw_line(&p1, &p2, &series_mask,
                                         &line_dsc_default);
                    ...
                    if (point_w && point_h
                        && ser->y_points[p_act] != LVX_CHART_POINT_NONE) {
                        lv_draw_rect(&point_area, &series_mask,
                                     &point_dsc_default);
                    }
...
static void draw_series_point_bar(lv_obj_t* obj, const lv_area_t* clip_area)
{
    ...
    _LV_LL_READ_BACK(&chart->series_ll, ser)
    {
        ...
        /* sort data */
        lv_coord_t point_cnt = ser->start_point;
...
static void draw_series_bar(lv_obj_t* obj, const lv_area_t* clip_area)
{
    ...
    for (i = 0; i < chart->point_cnt; i++) {
        ...
        _LV_LL_READ_BACK(&chart->series_ll, ser)
        {
            ...
            lv_event_send(obj, LV_EVENT_DRAW_PART_BEGIN, &part_draw_dsc);
            lv_draw_rect(&col_a_bg, &series_mask, &col_dsc_bg);
            lv_event_send(obj, LV_EVENT_DRAW_PART_END, &part_draw_dsc);
            part_draw_dsc.part = LV_PART_ITEMS;
#endif
            if (ser->y_points[p_act] != LVX_CHART_POINT_NONE) {
...
static void draw_y_ticks(lv_obj_t* obj, const lv_area_t* clip_area,
                         lvx_chart_axis_t axis)
{
    lvx_chart_t* chart = (lvx_chart_t*)obj;
...
static void draw_axes(lv_obj_t* obj, const lv_area_t* mask)
{
    draw_y_ticks(obj, mask, LVX_CHART_AXIS_PRIMARY_Y);
    draw_y_ticks(obj, mask, LVX_CHART_AXIS_SECONDARY_Y);
    draw_x_ticks(obj, mask, LVX_CHART_AXIS_PRIMARY_X);
    draw_x_ticks(obj, mask, LVX_CHART_AXIS_SECONDARY_X);
}
