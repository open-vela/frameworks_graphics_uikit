...
#define LVX_CHART_POINT_NONE (INT16_MAX)
LV_EXPORT_CONST_INT(LVX_CHART_POINT_NONE);

#define LVX_CHART_EXTENTIONS 1
 ...
 */
enum {
    LVX_CHART_TYPE_NONE, /**< Don't draw the series*/
    LVX_CHART_TYPE_LINE, /**< Connect the points with lines*/
...
/**
 * Chart update mode for `lvx_chart_set_next`
 */
enum {
    LVX_CHART_UPDATE_MODE_SHIFT, /**< Shift old data to the left and add the
                                     new one the right*/
    LVX_CHART_UPDATE_MODE_CIRCULAR, /**< Add the new data in a circular way*/
};
typedef uint8_t lvx_chart_update_mode_t;

/**
 * Enumeration of the axis'
 */
enum {
    LVX_CHART_AXIS_PRIMARY_Y = 0x00,
    LVX_CHART_AXIS_SECONDARY_Y = 0x01,
    LVX_CHART_AXIS_PRIMARY_X = 0x02,
    LVX_CHART_AXIS_SECONDARY_X = 0x04,
    _LVX_CHART_AXIS_LAST
};
typedef uint8_t lvx_chart_axis_t;
...
/**
 * `type` field in `lv_obj_draw_part_dsc_t` if `class_p = lvx_chart_class`
 * Used in `LV_EVENT_DRAW_PART_BEGIN` and `LV_EVENT_DRAW_PART_END`
 */
typedef enum {
    LVX_CHART_DRAW_PART_DIV_LINE_INIT, /**< Used before/after drawn the div
                                           lines*/
    LVX_CHART_DRAW_PART_DIV_LINE_HOR, /**< Used for each horizontal division
                                          lines*/
    LVX_CHART_DRAW_PART_DIV_LINE_VER, /**< Used for each vertical division
                                          lines*/
    LVX_CHART_DRAW_PART_LINE_AND_POINT, /**< Used on line and scatter charts
                                            for lines and points*/
    LVX_CHART_DRAW_PART_BAR, /**< Used on bar charts for the rectangles*/
    LVX_CHART_DRAW_PART_CURSOR, /**< Used on cursor lines and points*/
    LVX_CHART_DRAW_PART_TICK_LABEL, /**< Used on tick lines and labels*/
} lvx_chart_draw_part_type_t;
 ...
 * @param type      new type of the chart (from 'lvx_chart_type_t' enum)
 */
...
#if LVX_CHART_EXTENTIONS == 1
/**
 * Whether to draw a mask below the line.
 * @param obj           pointer to a chart object
 ...
 * @return          type of the chart (from 'lvx_chart_t' enum)
 */
 ...
 * (::LVX_CHART_AXIS_PRIMARY_Y or ::LVX_CHART_AXIS_SECONDARY_Y)
...
#if LVX_CHART_EXTENTIONS == 1
/**
 * Change the threshold of a series
 * @param obj       pointer to a chart object.
 * @param series    pointer to a series object
 * @param threshold the new threshold of the series
 */
 ...
 */
lvx_chart_series_t* lvx_chart_get_series_next(const lv_obj_t* chart,
 ...
 * @param point_id  the point's index or  `LVX_CHART_POINT_NONE` to not assign
 * to any points.
 */
 ...
 * @param obj       pointer to chart object
 * @param ser       pointer to a data series on 'chart'
 * @param value     the new value for all points. `LVX_CHART_POINT_NONE` can be
 ...
 * its index Can be used only with `LVX_CHART_TYPE_SCATTER`.
 * @param obj       pointer to chart object
 * @param ser       pointer to a data series on 'chart'
 * @param id        the index of the x point in the array
 * @param x_value   the new X value of the next data
